...
you should think that `λ = ∀`: so, the function `λ x, 3*x ^ 2 + 1` is nothing else that the
...
* `have h :` It allows to introduce a claim, called `h`. After, you need to prove it. There is also
...
-- * `self_eq_add_right : ∀ x, x =
The tactic `rw` can be used `at h` (where `h` is some local hypothesis) to perform the change there.
-/
...
* *sub_zero : ∀ x, x - 0 = x*
* *sub_self : ∀ x, x - x = 0*
Finally, some "associativity/distributivity" rules
...
theorem linear_add_cnst_of_affine (f : ℝ → ℝ) : is_affine f →
  (∃ a : ℝ, ∃ g : ℝ → ℝ, (f = g + (λ x, a)) ∧ is_linear g) :=
/- Observe that `(λ x, a)` is the _constant_ function taking the value `a` on every `x`-/
begin
  sorry
end
